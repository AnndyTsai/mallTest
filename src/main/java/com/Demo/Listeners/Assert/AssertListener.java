 ...
 *
 * 使用方法有两种：
 * todo 方式1：testng.xml配置文件中放置监听器
 * todo 方式2：类上放置监听器(@Listener)注解的方式引入监听器
 *
 *  * ***************************************备注*************************************
 *   1：该监听器重写了TestNG的Assert断言方法监听器，监听Assertion类，确保失败不抛出运行时异常
 *  * ***************************************结束*************************************
 * */
...
public class AssertListener extends TestListenerAdapter implements IAnnotationTransformer {

    /**
     * 断言失败重试
     *
     * todo：对于有重写TestNG assert断言的是不生效的
     * */
        ...
        if(!Assertion.flag){
            ...
            for (Error e : Assertion.errors) {
                ...
                StackTraceElement[] message = new StackTraceElement[]{new StackTraceElement
                        ("message : "+e.getMessage()+" in method : ",
